...
module.exports.index = (event, context, callback) => {
  ...
  co(function* () {
    ...
    const data = yield model.with((m) => co(function* () {
      return m.User.findAndCountAll({
        order: [['code', 'ASC']], offset, limit, where,
      });
    }));
...
module.exports.create = (event, context, callback) => {
  ...
  co(function* () {
    ...
    const data = yield model.with(
      (m) => m.User.create(params).then(
        (user) => m.User.findById(user.id),
      ),
    );
...
module.exports.show = (event, context, callback) => {
  ...
  co(function* () {
    ...
    const data = yield model.with(
      (m) => m.User.findById(id).then(verify.presence),
    );
...
module.exports.update = (event, context, callback) => {
  ...
  co(function* () {
    ...
    const params = JSON.parse(event.body);
    const data = yield model.with(
      (m) => m.User.findById(id).then(verify.presence).then(
        (u) => u.update(params),
      ),
    );
